 ...
 *      - in: formData
 *        email: email
 *        description: e-mail do usuario (unique)
 *        required: true
 *        type: string
 *      - in: formData
 *        password: password
 *        description: senha do usuarios (unique)
 *        type: string 
 *    responses:
 *      200:
 *        description: registro inserido com sucesso
...
app.post('/usuarios', (req, res) => {
  ...
  con.query(`SELECT * FROM usuario (email, password) VALUES ('${email}', '${password}')`, (err, result) => {
    if (err) {
      res.status(500)
      res.send(err)
      return
